...
/*   Updated: 2020/08/20 14:59:20 by fde-capu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_mat	*matrix_transpose(t_mat *a)
{
	...
	while (i <= a->m)
	{
		...
		{
			transposed->i = \
				lstdbl_addlast(transposed->i, matrix_get_element(a, i, j));
			j++;
		}
		i++;
	}
...
t_mat	*matrix_sum(t_mat *a, t_mat *b)
{
	...
	{
		summed->i = lstdbl_addlast(summed->i, matrix_get_element(a, c, 1) \
			+ matrix_get_element(b, c, 1));
		c++;
	}
	return (summed);
}
